...
IMUGPSFusionKF_2D_ConstantAcceleration::PushInnovationGPS(Vector6d& residual, Matrix6d& postiori_P) {
    ...
    if (this->m_l_GPS >= this->m_L_GPS) {
        ...
        this->m_R_GPS(2, 2) = epsilon;
        this->m_R_GPS(3, 3) = epsilon;
        this->m_R_GPS(4, 4) = epsilon;
        this->m_R_GPS(5, 5) = epsilon;

        this->m_R_GPS(0,0) = std::max(this->m_R_GPS(0,0), epsilon);
        this->m_R_GPS(1,1) = std::max(this->m_R_GPS(1,1), epsilon);
...
IMUGPSFusionKF_2D_ConstantAcceleration::PushInnovationIMU(Vector6d& residual,Matrix6d& postiori_P) {
    ...
    if (this->m_l_IMU >= this->m_L_IMU) {
        ...
        double epsilon = 1e-13;
        this->m_R_IMU(0, 0) = epsilon;
        this->m_R_IMU(1, 1) = epsilon;

        this->m_R_IMU(2,2) = std::max(this->m_R_IMU(2,2), epsilon);
        this->m_R_IMU(3,3) = std::max(this->m_R_IMU(3,3), epsilon);

        this->m_R_IMU(4,4) = std::max(this->m_R_IMU(4,4), epsilon);
        this->m_R_IMU(5,5) = std::max(this->m_R_IMU(5,5), epsilon);
...
IMUGPSFusionKF_2D_ConstantAcceleration::PushInnovationQ(Vector6d &posteriorResidual, Matrix6d &postiori_P) {
    if (this->m_posteriorResidualQueue.size() < this->m_N_Q) {
        this->m_innocationQueue_IMU.emplace_back(posteriorResidual);
        return;
    }
    ...
    if (this->m_l_Q >= this->m_L_Q) {
        ...
        double epsilon = 1e-13;
        this->m_Q(0,0) = std::max(this->m_Q(0,0), epsilon);
        this->m_Q(1,1) = std::max(this->m_Q(1,1), epsilon);
        this->m_Q(2,2) = std::max(this->m_Q(2,2), epsilon);
        this->m_Q(3,3) = std::max(this->m_Q(3,3), epsilon);
        this->m_Q(4,4) = std::max(this->m_Q(4,4), epsilon);
        this->m_Q(5,5) = std::max(this->m_Q(5,5), epsilon);
